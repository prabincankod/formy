...
export default async function SubmissionDetailPage({
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-8">
            ...
            <div>
                <p className="label-caps text-on-surface-variant mb-1">Submission Detail</p>
                ...
                <CardContent>
                    <dl className="divide-y divide-border-muted">
                        <div className="flex items-start gap-4 py-3">
                            <dt className="label-caps text-on-surface-variant w-32 shrink-0 pt-0.5">ID</dt>
                            <dd className="text-sm font-mono text-on-surface break-all">{submission.id}</dd>
                        </div>
                        {Object.entries(data).map(([key, value]) => (
                            <div key={key} className="flex items-start gap-4 py-3">
                                <dt className="label-caps text-on-surface-variant w-32 shrink-0 pt-0.5">{key}</dt>
